...
import { SetupDetails } from '../models/generalsetup.model';
...
export class SetupService {
  ...
  GetSalestax(obj) {
    ...
    // }
	getGeneralSetupDetails(storecode : String){
    try{
      ...
      return this.http.get<SetupDetails[]>(`${environment.baseUrl}DistributionSetup?`+'distributioncode='+ storecode
      ,{ headers: headers });
  ...
  updateGeneralSetupDetails(data: SetupDetails): Observable<any> {
    ...
    try {
      ...
      return this.http.put<any>(`${environment.baseUrl}DistributionSetup`,body,
       { headers: headers }) 
      ;
    }
    catch (error) {
...
DeleteUser(model) {
  ...
  return this.http.delete(`${environment.baseUrl}Users`, params).pipe(catchError(this.commonService.handleError));
}
}
